...
default package, and will load it right off the bat.
...
Looking at the text file directly, the appears to be a good amount of structure
...
row per participant, and add the two lines of raw text as strings as
...
adding a vector to tourny_data.
...
have a consistent border, which in my case they were a blank space. Also, in
...
pretournament rating, or NA. This is actually useful because now I can just
subset on the NAs and handle them directly.

As mentioned before, the main distinguishing characteristic of these records
is that the score is followed by an uppercase "P". Just like I used 
"lookbehind" above, I can use "lookahead" heare for a "P" without actually
extracting it.
...
the data is in a simple, consistent, unique pattern: any two digits followed
by a "|" on line 1. 
...
So now I have this 'opponents' list, but it does not actually contain what I
need, it only tells me where to look for what I need. Since each vector in the
list will need to follow the same process, I find the best way is to write a
function.
...
played by a player. Next, it instatiates a vector to keep track of the total 
pre tournament ratings of all opponents. 
...
total is divied by the number of opponents and returned, providing the average 
opponent score.
```{r}
...
I'll take a quick look at the head as a final check.
